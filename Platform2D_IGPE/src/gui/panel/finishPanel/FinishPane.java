...
public class FinishPane extends Pane{
	...
	public FinishPane(SinglePlayerPane s) {
		...
		statistics = new DataContainer(screen.getWidth()*0.45, screen.getHeight()*0.8, screen.getWidth()*0.55 - (screen.getWidth()*0.45/2), screen.getHeight()*0.22);
		...
		this.getChildren().addAll(background, title, clipboard_1, clipboard_2, (Node)statistics, restart, mainMenu, levelSelector,exitGame);
	}

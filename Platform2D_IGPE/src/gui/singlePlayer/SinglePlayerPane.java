...
public class SinglePlayerPane extends Pane {
	...
	private void updateCamera() {
		...
		if (manager.getPlayer().getX() < (manager.getLevelWidth() - 500) && manager.getPlayer().getX() > 300) {
			...
			scene.getCamera().setTranslateX(((manager.getPlayer().getX() < 300) ? 300 : (manager.getLevelWidth() - 500))
					- 500 + ((manager.getPlayer().getX() % 2 == 0) ? 0.1 : -0.1));
		...
		if (manager.getPlayer().getY() < (manager.getLevelHeight() - 200) && manager.getPlayer().getY() > 200) {
			...
			scene.getCamera()
					.setTranslateY(((manager.getPlayer().getY() < 200) ? 200 : (manager.getLevelHeight() - 200)) - 500
